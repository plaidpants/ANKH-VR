...
public class DayNightController : MonoBehaviour
{
    ...
    [SerializeField] public float AtmosphereAdjustment = 0.5f;
    [SerializeField] public float FogAdjustment = 0.007f;
    ...
    private Renderer starParticles;

    void Start()
    {
        starParticles = starsTransform.GetComponent<ParticleSystem>().GetComponent<Renderer>();
    ...
    void Update()
    {
        ...
        starsTransform.Rotate(direction, rotation - prev_rotation);
        ...
        {
            intensity = intensityAtSunSet + (intensityAtNoon - intensityAtSunSet) * (time - sunRise) / (43200f - sunRise);
        }
        else if (time < sunSet)
        {
        ...
        if (Time_Falls_Between(time, timeLight, timeExtinguish) == true)
        {
        ...
        tintColor.a = fade;
        starParticles.material.SetColor("_TintColor", tintColor);

        // turn the atmosphere off at night
        RenderSettings.skybox.SetFloat("_AtmosphereThickness", AtmosphereAdjustment * (1f - fade));

        // turn the fog off at night
        RenderSettings.fogDensity = FogAdjustment * (1f - fade);
    }
