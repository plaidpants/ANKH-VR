...
C_9209()
{
	...
	if(CurMode == MOD_OUTDOORS && tile_cur == TIL_17 && U4_RND1(7) == 0 && Party.f_1dc != 1) {	// Added and not flying in the balloon
		u4_puts(/*D_2998*/&AVATAR[0x11C43 + 0x0017] /* "\nBridge Trolls!\n" */);
		D_96F8 = TIL_A4;
		D_9772 = Party._x;
		D_9140 = Party._y;
		D_946C = TIL_17;
	...
	if(CurMode == MOD_DUNGEON) {
		if((tile_cur & 0xf0) == 0xa0) {
			...
			if(tile_cur == 0x80) {
				u4_puts(/*D_29A9*/&AVATAR[0x11C54 + 0x0017] /* "\nWinds!\n" */);
				Party.f_1dc = 0;
			} else {
				u4_puts((tile_cur < 0x88)?/*D_29B2*/&AVATAR[0x11C5D + 0x0017] /* "\nFalling Rocks!\n" */:/*D_29C3*/ &AVATAR[0x11C6E + 0x0017] /* "\nPit!\n" */);
				C_1584();
			}
		}
		return 0;
