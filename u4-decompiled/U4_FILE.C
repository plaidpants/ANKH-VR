...
int /*C_0855*/DisableLoad(char* filename, int filesize, char * filebuffer)
{
	...
	if((byteswritten = fread(filepointer, 1, filebuffer, filesize)) == -1 || filesize != byteswritten) {
		Console(&AVATAR[0xF728 + 0x0005] /* "\r\nReading " */);
		if(byteswritten == -1) {
			Console(filename);
		} else {
			Console(filename);
			Console(&AVATAR[0xF733 + 0x0005] /* ": File wrong size\r\n" */);
		}
		return -1;
	}
	if(fclose(filepointer) == -1) {
		Console(&AVATAR[0xF747 + 0x0005] /* "\r\nClosing " */);
		Console(filename);
		return -1;
	}
#endif
